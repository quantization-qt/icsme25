...
SONAR_TOKEN = '34dfbdf056e7b9de2ae592a14798662009545380'  # Replace with your SonarCloud token
PROJECT_KEY = 'saimaafrin_sonarQube-analysis'       # Replace with your SonarCloud project key
SONAR_URL = 'https://sonarcloud.io'

# Directory to filter (relative to project root)
#TARGET_DIRECTORY = 'mc-py-codellama13B-AWQ-all'  # Example: 'py-codellama7B-AWQ-all'
TARGET_DIRECTORY = 'py-deepseek33B-AWQ-all'  
...
# Sort taskids numerically -humaneval
...
'''def extract_taskid(file_path):
    ...
    match = re.search(r'Python-(\d+)-gen\d+\.py', filename)  # Match pattern "Python-10-gen0.py"
    if match:
        taskid = f"Python-{match.group(1)}"
        return taskid
    return None

# Sort taskids numerically -mceval
def sort_taskids(taskid_list):
    def extract_number(taskid):
